#assessing size distributions of two species
library(ggplot2)
library(cowplot)
...
#adding extra column for size index, calling it cuke_sizes (base df is cuke_size)
...
cuke_size %>% 
  group_by(species) %>% 
  summarise(ci = list(mean_cl_normal(dist_length_cm) %>% 
                        rename(mean=y, lwr=ymin, upr=ymax))) %>% 
  unnest(cols=c(ci))

  summarise(mean(dist_length_cm))
...
# size index is standard calulation that incorporates length and width of sea cucumbers
...
#### MSc CH1 Fig 2 - Length histogram instead of size index - Sept 24 2020 ####
pt_1 <- ggplot(cuke_sizes, aes(x=dist_length_cm, fill = species, color=species), binwidth = 0.5) +
 ...
 #### FIGURE in PEERJ Manuscript, Nov 2022, (and defense) ####
 pt_2 +
  theme(  
   panel.background = element_rect(fill = "transparent",colour = NA),
   panel.grid.minor = element_blank(),
...
##_____________  Kolmogorov-Smirnov test function ______________________________
...
#model w size index and dist_lengh_cm return same p-vals
