...
pred_weights <- read_csv("predicted_weights.csv") %>%
  filter(treatment!='NA') %>%
  # 12 individuals we don't have data for, so remove those
  filter(species!='NA') %>%
  # only want to consider the cucumbers "kept" or "removed", ignore the ones that were "removed" 
  # FOR READERS: Cucumbers "kept" or "added" were individuals that fell on or within the dense seagrass halo 
  # around patch reefs that we measured. Cucumbers that were "removed" were those who fell outside of the dense seagrass
  # halo, and were therefore not considered in the analysis. 
...
##### CH1 FIG S1 panel x2 March 4 2021#####
library(cowplot)
plot_grid(graph_test3excr, graph_test3331)


######### PEERJ Supplementary Fig 3####
...
library(here)
get_se_WR2 <- read_csv("get_se_WR_v2.csv")
